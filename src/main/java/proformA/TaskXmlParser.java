...
/**
 * Die Klasse {@code TaskXmlParser} extrahiert Kriterien und Musterlösungen aus
...
public class TaskXmlParser {
	 ...
	 */
	public List<CriterionWithWeight> extractCriteriaWithWeights(String taskXmlPath, boolean debug)
	...
	private void processGradingHints(GradingHintsType gradingHints, List<CriterionWithWeight> criteriaWithWeights,
		...
		if (debug)
			System.out.println("Verarbeite GradingHints: root=" + gradingHints.getRoot() + ", combine="
					+ gradingHints.getCombine());
		...
		if (refs != null) {
			for (GradesBaseRefChildType ref : refs) {
				...
				if (ref instanceof GradesTestRefChildType) {
					...
					String refId = testRef.getRef(); // refId aus der XML extrahieren
					double weight = testRef.getWeight() != null ? testRef.getWeight().doubleValue() : 0.0;
					if (debug)
						System.out.println(
								"TestRef gefunden: subRef=" + subRef + ", refId=" + refId + ", weight=" + weight);
					if (subRef != null && CRITERIA_MAPPING.containsKey(subRef)) {
						...
						criteriaWithWeights.add(new CriterionWithWeight(criterion, weight, refId)); // refId hinzufügen
					}
	 ...
	 * @param debug       Flag zur Aktivierung von Debugging-Ausgaben
	 * @return Byte-Array der Musterlösung (z. B. XMI-Datei), oder null wenn keine
	 *         Musterlösung gefunden wurde
	...
	public byte[] extractReferenceModel(String taskXmlPath, boolean debug) throws JAXBException, IOException {
		if (debug)
			System.out.println("Versuche, Musterlösung aus " + taskXmlPath + " zu extrahieren");
		...
		if (modelSolutions != null && modelSolutions.getModelSolution() != null) {
			if (debug)
				System.out
						.println("ModelSolutions gefunden: " + modelSolutions.getModelSolution().size() + " Lösungen");
			for (ModelSolutionType solution : modelSolutions.getModelSolution()) {
				if (debug)
					System.out.println("Verarbeite ModelSolution: " + solution);
				// Finde die referenzierte Datei
				if (solution.getFilerefs() != null && !solution.getFilerefs().getFileref().isEmpty()) {
					if (debug)
						System.out.println("Filerefs gefunden: " + solution.getFilerefs().getFileref().size());
					for (FilerefType fileref : solution.getFilerefs().getFileref()) {
						String refId = fileref.getRefid(); // Korrigierter Methodenname
						if (debug)
							System.out.println("Verarbeite Fileref mit refId: " + refId);
						// Suche die Datei in <files>
						if (task.getFiles() != null && !task.getFiles().getFile().isEmpty()) {
							if (debug)
								System.out.println("Files gefunden: " + task.getFiles().getFile().size());
							for (TaskFileType file : task.getFiles().getFile()) {
								if (file.getId().equals(refId)) {
									if (debug)
										System.out.println("Passende Datei gefunden mit id: " + refId);
									// Prüfe auf <embedded-txt-file>
									if (file.getEmbeddedTxtFile() != null) {
										String content = file.getEmbeddedTxtFile().getValue(); // Korrigierter
																								// Methodenname
										if (debug)
											System.out.println("EmbeddedTxtFile gefunden, Inhalt: " + content);
										return content.getBytes(); // Text als bytes zurückgeben
									}
									// Prüfe auf <embedded-bin-file>
									if (file.getEmbeddedBinFile() != null) {
										byte[] content = file.getEmbeddedBinFile().getValue(); // Korrigierter
																								// Methodenname
										if (debug)
											System.out.println("EmbeddedBinFile gefunden, Länge: " + content.length);
									...
									if (file.getAttachedBinFile() != null) {
										...
										if (fileOnDisk.exists()) {
											if (debug)
												System.out.println("AttachedBinFile gefunden, Pfad: " + filePath);
											return java.nio.file.Files.readAllBytes(fileOnDisk.toPath()); // Externe
																											// Datei
																											// lesen
										} else {
											if (debug)
												System.out.println("AttachedBinFile nicht gefunden unter: " + filePath);
										}
									}
								}
							}
			...
			if (debug)
				System.out.println("Keine ModelSolutions gefunden");
		}
